...
def _parse_sm70_moe_dense_allowlist() -> set[str] | None:
    raw = os.getenv("VLLM_SM70_MOE_DENSE_ALLOWLIST")
    if raw is None:
        return None
...
def _uses_split_gdn_input_projections(
    ...
    Qwen3.5-27B-AWQ stores:
      - linear_attn.in_proj_qkv / in_proj_z in AWQ form
      - linear_attn.in_proj_a / in_proj_b in BF16

    That layout cannot be represented by a single fused quantized
    ``in_proj_qkvz`` module, so we keep ``qkvz`` and ``ba`` as separate packed
    projections for this case only.
    """
    ...
    if not modules_to_not_convert:
        # INC/auto-round exposes per-layer precision via `extra_config`, NOT
        # modules_to_not_convert. AutoRound leaves in_proj_a/b in fp16 (bits>=16)
        # when their shape isn't divisible by 32 (e.g. the 27B), which can't be
        # fused with quantized qkv/z. Detect that here so the split path is used;
        # otherwise the fused path silently corrupts the fp16 b/a gating -> garbage.
        extra = getattr(quant_config, "extra_config", None)
        if extra:
            return any(
                ("linear_attn.in_proj_a" in n or "linear_attn.in_proj_b" in n)
                and isinstance(c, dict) and c.get("bits", 16) >= 16
...
def _get_default_sm70_dense_force_suffixes(tp_size: int) -> set[str]:
    # Qwen3.5-27B on V100 benefits most consistently from the narrower
    # Qwen3Next projection pair. Enabling the broader in-proj set regresses
    # TP2 latency and reintroduces the compile/int input failure on TP4.
    _ = tp_size
    import os as _os
    if _os.getenv("VLLM_SM70_NO_DENSE") == "1":
        return set()   # keep weights quantized (fp16 dense-prepare OOMs the 27B on one card)
    return {
        "qkv_proj",
        "out_proj",
    }
...
class Qwen3_5GatedDeltaNet(Qwen3NextGatedDeltaNet):
    ...
    def create_qkvz_proj(
        ...
        if _uses_split_gdn_input_projections(quant_config):
    ...
    def forward(
        ...
        if self.use_split_input_projections:
            ...
            # Match the native Qwen3.5 GDN layout:
            #   qkvz = [q, k, v, z]
            #   ba   = [b, a]
            ...
            mixed_qkvzba = _maybe_sm70_projection(self.in_proj_qkvz, hidden_states)
            if mixed_qkvzba is None:
                mixed_qkvzba = self.in_proj_qkvz(hidden_states)
            mixed_qkvzba, _ = mixed_qkvzba
            qkv_size = (self.key_dim * 2 + self.value_dim) // self.tp_size
            z_size = self.value_dim // self.tp_size
            ba_size = self.num_v_heads // self.tp_size
            ...
            # Normalize split views so torch.compile/cudagraph do not
            # specialize on the fused projection's parent stride.
            mixed_qkv = mixed_qkv.contiguous()
            z = z.contiguous().reshape(z.size(0), -1, self.head_v_dim)
            b = b.contiguous()
            a = a.contiguous()
...
class Qwen3_5Model(Qwen3NextModel):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.embed_tokens = VocabParallelEmbedding(
            self.vocab_size,
            config.hidden_size,
            quant_config=vllm_config.quant_config,   # enables quantized token embedding (GPTQEmbeddingMethod)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # as separate weights. Dense AWQ checkpoints mix precision here:
        ...
        stacked_params_mapping = [
            ...
            # GDN qkv/z are always packed together.
...
class Qwen3_5ForCausalLMBase(
    ...
    packed_modules_mapping = {
        ...
        # GDN projections can be mixed-precision on dense Qwen3.5 AWQ
        # checkpoints, so keep qkv/z and b/a as separate packed groups.
        ...
        if get_pp_group().is_last_rank:
            if config.tie_word_embeddings:
                ...
                self.lm_head = ParallelLMHead(
                    config.vocab_size,
                    config.hidden_size,
                    quant_config=self.quant_config,  # enables a quantized (e.g. 8-bit) lm_head via GPTQ
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        loader = AutoWeightsLoader(
            self,
            skip_prefixes=(["mtp.", "model.visual.", "visual."] if getattr(self.config, "language_model_only", False) else ["mtp."]),
        )
...
class Qwen3_5MoeForCausalLM(Qwen3_5ForCausalLMBase, QwenNextMixtureOfExperts):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # The experimental SM70 dense decode fast path is only safe on the
        # dense 27B line today. Force-disable it for Qwen3.5 MoE variants so a
...
class Qwen3_5ForConditionalGeneration(Qwen3VLForConditionalGeneration, IsHybrid):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "model"):
        ...
        if getattr(config, "language_model_only", False):
            self.visual = None  # text-only serving: skip vision tower (~0.9GB + encoder cache)
        else:
